...
function _levi_subroot_system_and_lambdas_restricted(
  ...
  lambdas_restricted = map(lambdas) do lambda
    ...
    sum(
      coeffs[indices_of_S[i]] * fundamental_weight(levi_root_system, i) for i in range_S;
  ...
  for i in 1:length(reprs)
    ...
    for t in pos_roots
      ...
      if j > i
        ...
        vec_coeffs = coefficients(t * inv_omega) * gen_matrix
...
function _tautological_bd(
  ...
  lambdas_dynamic_vals = _lambdas_dynamic(lambdas, indices_of_S)
  ...
  dc_m_map = map(lambdas_dynamic_vals) do ld
    get!(dc_m_cache, ld) do
      dominant_character(R, ld)
    end
  end
  ...
  # Using map to preserve shape of lambdas
  dc_o_map = map(lambdas, lambdas_dynamic_vals, dc_m_map) do lambda, l_dyn, l_dc_m
    lambda_static = lambda - l_dyn
    D = Dict{WeightLatticeElem,Vector{WeightLatticeElem}}()

    for l in keys(l_dc_m)
      # Reverted logic: l * w + lambda_static
      orbit = [l * w + lambda_static for w in WP]
      ...
      for f in orbit_fiber
        ...
        for _ in 1:multiplicity
          ...
          for (i, omega) in enumerate(reprs)
            ...
            c = coefficients(sum(k -> dot(t, fundamental_weight(R, k)) * simple_root(R, k), 1:rank_R))
            vec_coeffs = c * gen_matrix
  ...
  return map(CartesianIndices(lambdas)) do I
