...
function _multiplicities(H2::GKM_H2, edge_list::Vector{Edge}, beta::CurveClass)
  ...
  h2_rank = rank(H2.H2)
  # The direct DFS is highly effective in rank one, but its cone test is too
  # weak in higher rank and can explore a large search tree. Preserve the
  # original Polymake solver there until a rank-aware elimination solver is
  # available.
  h2_rank == 1 || return _multiplicities_polymake(H2, edge_list, beta)
  ...
  for (i, edge) in enumerate(edge_list)
    ...
    edge_classes[i] = ZZRingElem[image[j] for j in 1:h2_rank]
  end

  beta_coordinates = ZZRingElem[beta[j] for j in 1:h2_rank]
  ...
  dual_rays = [QQFieldElem[ray[j] for j in 1:h2_rank] for ray in rays(H2.dual_cone)]
