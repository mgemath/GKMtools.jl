...
function _lift(R, indices_of_S, lambda_res)
  ...
  # coe = Oscar.coefficients(lambda_res) * transpose(inv(matrix(QQ, cartan_matrix(levi_root_system)))) # need transpose
...
function rank_of_bd(lambdas::AbstractArray{WeightLatticeElem}, indices_of_S::AbstractVector{<:Integer}=Int[])
  _same_root_system(lambdas)
  R = root_system(first(lambdas))
...
function rank_of_bd(lambda::WeightLatticeElem, indices_of_S::AbstractVector{<:Integer}=Int[])
...
function _tautological_bd(
  ...
  for i in eachindex(lambdas)
    ...
    lambda_static = Oscar.coefficients(lambda) - _lift(R, indices_of_S, lambda_res)# this is the part of lambda that is not captured by the Levi, we need to add it back to the lifted weight to get the full weight.
  ...
  function compute_single_bundle(index, lambda)
    ...
    sorted_keys = sort(collect(keys(l_dc_o)); by=w -> sum(Oscar.coefficients(w)))
    ...
    for (i, omega) in enumerate(reprs)
      ...
      for l in sorted_keys #keys(l_dc_o)# 
        ...
        for f in orbit_fiber
          ...
          # c = Oscar.coefficients(t) * transpose(inv(matrix(QQ, cartan_matrix(R)))); println("Coefficients for t: ", Oscar.coefficients(t), " are: ", c)
          # c = Oscar.coefficients(sum(k -> dot(t, fundamental_weight(R, k)) * simple_root(R, k), 1:rank_R)); println("Coefficients after conversion to simple roots: ", c)
    ...
    M_bd = M
    GMtoM = hom(M, M_bd, gens(M_bd))

    return vector_bundle(GP, M_bd, GMtoM, weightMatrix_S)
  end
