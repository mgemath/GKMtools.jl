...
Print the moment graph of the Bott-Samelson desingularization associated with the sequence of simple roots `S`. Set `printEdges=false` to print only the summary.

# Example
```jldoctest
julia> R = root_system(:A, 1);

julia> bott_samelson([simple_root(R, 1)]; printEdges=false)
Bott samelson graph in type Root system of type A1 defined by RootSpaceElem[a_1]
Is GKM: true
Number of vertices: 2
Number of edges: 1
Number of fat edges: 0
```
"""
function bott_samelson(S::Vector{RootSpaceElem}; printEdges::Bool = true)
  ...
  @req length(S) > 0 "Need at least one root for Bott-Samelson variety."
  R = root_system(S[1])
