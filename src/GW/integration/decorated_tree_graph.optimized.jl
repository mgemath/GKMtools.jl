...
function edgeMult(e::Edge, tree::GW_decorated_tree)
  # SimpleGraph edges are stored in canonical orientation. Canonicalizing once
  # avoids two dictionary probes for every flag evaluated by Euler_inv.
  canonical_edge = src(e) < dst(e) ? e : reverse(e)
  @req haskey(tree.edgeMult, canonical_edge) "edge has no multiplicity assigned in decorated tree"
  return tree.edgeMult[canonical_edge]
end
