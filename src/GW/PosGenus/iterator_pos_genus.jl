...
function compute_threshold_for_progress_bar(max_genus::Int64, max_edges::Int64)::Int64
  ...
  threshold::Int64 = sum(vert -> number_trees[vert] * sum(genus -> length(weak_compositions(genus, vert)), 0:max_genus), 2:(max_edges + 1))
...
function genus_distribution_mod_iso(top_graph_Graphs::Graphs.SimpleGraph{Int64}, top_aut::Int64, max_genus::Int64, genus::Int64)

  ((top_aut == 1) || (max_genus == genus)) && return Iterators.zip(map(x -> collect(x), weak_compositions(max_genus - genus, Graphs.nv(top_graph_Graphs))), Iterators.cycle([top_aut]))

  return Iterators.flatmap(p -> unique_gen_dist_fixed_partition(top_graph_Graphs, p), Combinatorics.partitions(max_genus - genus))
end

function unique_gen_dist_fixed_partition(top_graph_Graphs::Graphs.SimpleGraph{Int64}, p::Vector{Int64})
  ...
  for perm_extended_p in Combinatorics.multiset_permutations(extended_p, Graphs.nv(top_graph_Graphs))
    ...
    for (gen_dist, gen_dist_aut) in ans
      ...
      color_1(v, u) = (perm_extended_p[v] == gen_dist[u])
    ...
    if !found # new coloring if not found
      color_rel_2(u, v) = (perm_extended_p[v] == perm_extended_p[u])
...
function colorings_modulo_iso(top_graph_Graphs::Graphs.SimpleGraph{Int64}, nc::Dict{Int64,Vector{Int64}}, gen_dist::Vector{Int64}, gen_dist_aut::Int64)
  
  return Iterators.flatmap(comb -> unique_col_fixed_combination_w_counting(top_graph_Graphs, nc, gen_dist, gen_dist_aut, comb), Combinatorics.with_replacement_combinations(1:length(nc), Graphs.nv(top_graph_Graphs)))
end

function unique_col_fixed_combination_w_counting(top_graph_Graphs::Graphs.SimpleGraph{Int64}, nc::Dict{Int64,Vector{Int64}}, gen_dist::Vector{Int64}, gen_dist_aut::Int64, comb::Vector{Int64})::Set{Tuple{Vector{Int64}, Int64}}
  ...
  for c in Combinatorics.multiset_permutations(comb, length(comb))
    ...
    for (color2, col_aut) in ans

      seen[color2] == 0 && continue # already used all the copies of this coloring
      color_rel(u, v) = (c[u] == color2[v]) && (gen_dist[v] == gen_dist[u]) # vertex relation for isomorphism check
      
      if (gen_dist_aut > 1) && Graphs.Experimental.has_isomorph(top_graph_Graphs, top_graph_Graphs, vertex_relation=color_rel) # check isomorphism
        found = true
        seen[color2] -= 1 # use one copy of this coloring
        break
      end
    ...
    if !found # new coloring if not found      
      ...
      aut = 1
      if gen_dist_aut > 1
        color_rel_2(u, v) = (c[u] == c[v]) && (gen_dist[v] == gen_dist[u]) # vertex relation for isomorphism check
        aut = Graphs.Experimental.count_isomorph(top_graph_Graphs, top_graph_Graphs, vertex_relation=color_rel_2) # count automorphisms of the coloring
      end

      push!(ans, (c, aut))

      divis = div(gen_dist_aut, aut)
...
function multiedges_mod_iso(top_graph_Graphs::Graphs.SimpleGraph{Int64}, top_graph::Graph{Undirected}, gen_dist::Vector{Int64}, col::Vector{Int64}, col_aut::Int64, max_genus::Int64, top_genus::Int64, beta, H2)
  
  return Iterators.flatmap(edge_mult_plus_edge_mult_aut -> all_multiedges_fixed_multi_mod_iso(top_graph_Graphs, top_graph, gen_dist, col, edge_mult_plus_edge_mult_aut[1], edge_mult_plus_edge_mult_aut[2], max_genus, top_genus), unique_edge_multi_mod_iso(top_graph_Graphs, top_graph, gen_dist, col, col_aut, beta, H2))
end
  ...
  for array_number_multiedges = Combinatorics.multiset_permutations(p, Graphs.ne(top_graph_Graphs)) # distribute the parts of p to the edges
    ...
    for multi_edges in Iterators.product([Combinatorics.partitions(edge_mult_dict[e], edge_mult_number_multiedges[e]) for e in edges(top_graph)]...)
      ...
      for (multi_edges2, multiedges_aut) in ans
        ...
        if (edge_multi_aut > 1) && Graphs.Experimental.has_isomorph(top_graph_Graphs, top_graph_Graphs, vertex_relation=vertex_rel, edge_relation=edge_rel) # check isomorphism
          found = true
...
function unique_edge_multi_mod_iso(top_graph_Graphs::Graphs.SimpleGraph{Int64}, top_graph::Graph{Undirected}, gen_dist::Vector{Int64}, col::Vector{Int64}, col_aut::Int64, beta, H2)
  ...
  for edgeMult_array in Multi # iterate edge multiplicities
    ...
    for (edgeMult_array2, edge_mult_aut) in ans
      ...
      if (col_aut > 1) && Graphs.Experimental.has_isomorph(top_graph_Graphs, top_graph_Graphs, vertex_relation=vertex_rel, edge_relation=edge_rel) # check isomorphism
        found = true
        # seen[color2] -= 1 # use one copy of this coloring
        break
      end
