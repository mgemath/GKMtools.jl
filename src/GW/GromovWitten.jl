...
```
"""
function gromov_witten(G::GKM_graph, beta::CurveClass, n_marks::Int64, P_input::EquivariantClass; show_bar::Bool = true, check_degrees::Bool = false, fast_mode::Bool = false)
  return gromov_witten(G, beta, n_marks, [P_input]; show_bar=show_bar, check_degrees=check_degrees, fast_mode)[1]
end

function gromov_witten(G::GKM_graph, beta::CurveClass, n_marks::Int64, P_input::Array{EquivariantClass}; show_bar::Bool = true, check_degrees::Bool = false, fast_mode::Bool = false)
            ...
            if is_zero(euler) #euler == zero(R.coeffRing)
              ...
              for e in edges(tree)
                ...
                if !haskey(h_dict, triple)
                    h_dict[triple] = _h(Edge(col[src(e)], col[dst(e)]), triple[1], G; check=false, check_degrees=check_degrees)
                end
                euler *= h_dict[triple]
              end
...
# @doc raw"""
#     gromov_witten(V::GKM_vector_bundle, beta::CurveClass, n_marks::Int64, P_input::EquivariantClass; show_bar::Bool = false, check_degrees::Bool = false)

# Same as before, but taking the total space of a GKM vector bundle as input.
# """
# function gromov_witten(V::GKM_vector_bundle, beta::CurveClass, n_marks::Int64, P_input::EquivariantClass; show_bar::Bool = false, check_degrees::Bool = false)
...
# Same as before, but taking the total space of a GKM vector bundle as input.
# """
# function gromov_witten(V::GKM_vector_bundle, beta::CurveClass, n_marks::Int64, P_input::Array{EquivariantClass}; show_bar::Bool = true, check_degrees::Bool = false)
