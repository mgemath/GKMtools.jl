export store_colors, gromov_witten_load_color, gromov_witten_nomarks_load_color, gromov_witten_nomarks_modulo
...
function gromov_witten_nomarks_modulo(G::AbstractGKM_graph, beta::CurveClass_type, evClasses::Vector{FreeModElem{QQMPolyRingElem}}, modulo::Int64; show_bar::Bool = true, check_degrees::Bool = false, fast_mode::Bool = false, g::Int64 = 0)
  # res = gromov_witten_nomarks_load_color(G, beta, [evClasses], line_num; show_bar=show_bar, check_degrees=check_degrees, fast_mode=fast_mode, g=g)[1]
  # namefile = res == 0 ? "$(line_num)_z.txt" : "$line_num.txt"
  # f = open(namefile, "w")
  # println(f, res)
...
function gromov_witten_nomarks_modulo(G::AbstractGKM_graph, beta::CurveClass_type, evClasses::Vector{Vector{FreeModElem{QQMPolyRingElem}}}, modulo::Int64; show_bar::Bool = true, check_degrees::Bool = false, fast_mode::Bool = false, g::Int64 = 0)
  ...
  for ls in Iterators.flatten([TreeIt(i) for i in 2:max_n_vert]) # generation of level sequences
    ...
    (isfile("$(counter).txt") || isfile("$(counter)_z.txt")) && continue
    ...
    namefile = res[1] == 0 ? "$(counter)_z.txt" : "$counter.txt"
    f = open(namefile, "w")
    print(f, res[1])
    close(f)
