...
struct OrbifoldVertexIsotropy <: AbstractVertexIsotropy{Vector{Int}, ZZMatrix}
  isotropy_group::Vector{Int}  # group structure of the isotropy group at the vertex
  tangent_rep::ZZMatrix     # full tangent representation
end
...
function order_of_isotropy_group(I::AbstractIsotropy{Vector{Int}, Rep}) where {Rep}
...
function smooth_orbifold_vertex_isotropy_group(d)
  return OrbifoldVertexIsotropy(Int[], zero_matrix(ZZ, 0, d))
end

function smooth_orbifold_flag_isotropy_group(d, k)
  # return OrbifoldFlagIsotropy(Int[], zeros(Int, 0, d), identity_matrix(ZZ, k))
  return OrbifoldFlagIsotropy(Int[], identity_matrix(ZZ, k))
end

issmooth(I::AbstractIsotropy) = isempty(I.isotropy_group)
